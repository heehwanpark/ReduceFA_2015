...
Example:
    conf = optim.ConfusionMatrix_HH( {'cat','dog','person'} )   -- new matrix
...
local ConfusionMatrix_HH = torch.class('ConfusionMatrix_HH')

function ConfusionMatrix_HH:__init(nclasses, classes)
...
function ConfusionMatrix_HH:_add(p, t)
...
function ConfusionMatrix_HH:add(prediction, target)
...
function ConfusionMatrix_HH:batchAdd(predictions, targets)
...
function ConfusionMatrix_HH:zero()
   self.mat:zero()
   self.valids:zero()
...
function ConfusionMatrix_HH:updateValids()
   local total = 0
...
function ConfusionMatrix_HH:farFrr()
   local cmat = self.mat
...
function ConfusionMatrix_HH:__tostring__()
   self:updateValids()
   local str = {'ConfusionMatrix_HH:\n'}
...
function ConfusionMatrix_HH:render(sortmode, display, block, legendwidth)
   -- args
