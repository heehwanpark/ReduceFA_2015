...
-- File name
cmd:option('-filename', '0826_result_10f')
...
----------------------------------------------------------------------
for fold_index = 1, 10 do
  print('10 Fold validation')
  print('Currnet fold is ' .. fold_index .. '\n')
  ...
  elseif option.datatype == 'mitdb' then
    ...
    mitdb_input = torch.cat(mitdb_tr_input, mitdb_te_input, 1)
    mitdb_target = torch.cat(mitdb_tr_target, mitdb_te_target, 1)
    ...
    for i = 1, 46 do
      ...
      if i <= 41 then
        if i == 1 then
          ...
          mitdb_tr_input = mitdb_tr_input:cat(selected_input, 1)
        ...
        if i == 42 then
          ...
          mitdb_te_input = mitdb_te_input:cat(selected_input, 1)
          mitdb_te_target = mitdb_te_target:cat(selected_target, 1)
        end
      end
    end
  else
  ...
  if option.pretraining then
    recordfile = hdf5.open(folder .. option.filename .. '_' .. option.datatype .. '_' .. fold_index .. '_wi_pre.h5', 'w')
  else
    recordfile = hdf5.open(folder .. option.filename .. '_' .. option.datatype .. '_' .. fold_index .. '_wo_pre.h5', 'w')
  end
